resource "aws_lambda_function" "cognito_custom_message_lambda" {
  ...
  function_name    = "${var.environment_name}-${var.service_name}-custom-message-lambda-${data.terraform_remote_state.region.outputs.aws_region_shortname}"
...
resource "aws_iam_role" "cognito_custom_message_lambda_role" {
  name = "${var.environment_name}-${var.service_name}-custom-message-lambda-role-${data.terraform_remote_state.region.outputs.aws_region_shortname}"
